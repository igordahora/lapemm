...
class PgSqlPdoStrategy {
            ...
            if ($mutiplasColunas) {
                ...
                foreach ($arrayOrderBy as $orderByAtributo) {
                    ...
                    if ($order) {
                        ...
                        $order = $dados[1];
                        $colunas[] = "lower(to_ascii({$colmap},'LATIN1')) {$order}";
                    } else {
                        $colmap = $this->propAtributos[$orderByAtributo]['Colmap'];
                        $colunas[] = "lower(to_ascii({$colmap},'LATIN1'))";
                    }
                ...
                if ($order) {
                    ...
                    $colmap = $this->propAtributos[$dados[0]]['Colmap'];
                    $order = $dados[1];
                    $orderby = "ORDER BY lower(to_ascii({$colmap},'LATIN1')) {$order}";
                    unset($dados);
                } else {
                    $colmap = $this->propAtributos[$orderby]['Colmap'];
                    $orderby = "ORDER BY lower(to_ascii({$colmap},'LATIN1'))";
                }
    ...
    public function listar($where = null, $orderby = null, $objectCollection = null, $exception = null, $offset = null) {
        ...
        $query = "SELECT {$this->colunas} FROM {$this->schema}.{$this->tabela} {$where} {$orderby} {$offset}"; //echo $query; exit();
    ...
    public function salvar($dados, $objectResult = null, array $exception = null) {
        ...
        if (is_array($dados)) {
            ...
            $result[0] = false;
            $result[1] = $dados;
            $result[2]['post'] = 'Nгo pode ser salvo (Para salvar precisa passar um objto ou um array)';
            return $result;
        }
